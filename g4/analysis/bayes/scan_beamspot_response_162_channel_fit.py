#!/usr/bin/env python3
"""Beam-spot response scan for the 162-keV Dalitz fit.

This standalone script scans the effect of beam-spot size by fitting a
series of mixed closure ROOT files with different beam FWHM values.

Common analysis conditions:
    - full laboratory angular coverage;
    - exactly N_fit accepted complete three-alpha events at every point;
    - pure benchmark templates remain ideal continuous-angle templates
      reconstructed from momentum;
    - each mixed closure file is analyzed with user-chosen closure-only
      angular granularity (Delta-theta, Delta-phi);
    - closure energy smearing and threshold may also be applied;
    - mixed closure directions are reconstructed from the virtual-sphere hit
      position relative to the nominal sphere centre, except for the special
      baseline case of point beam with Delta-theta = Delta-phi = 0, where
      momentum angles are used to give the exact ideal reference.
...
SCRIPT_VERSION = "2026-07-20-beamspot-scan-v3-fixed-y-ranges"
...
def parse_args(argv=None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Scan the effect of closure beam-spot size on the extracted 162-keV "
            "channel fractions. Pure benchmark templates remain ideal "
            "continuous-angle templates reconstructed from momentum. "
            "Each mixed closure ROOT file is treated as one beam-spot point."
        )
    )
    parser.add_argument(
        "seq_be_gs",
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "one or more mixed closure ROOT files with different beam spots; "
            "filenames are sorted by beam FWHM inferred from 'beam_fwhmXmm'. "
            "A file without that token is treated as point beam (0 mm)."
        ),
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="closure polar-angle granularity Delta-theta (default: 0 deg)",
    )
    parser.add_argument(
        "--phi-granularity-deg",
        type=float,
        default=0.0,
        help="closure azimuthal-angle granularity Delta-phi (default: 0 deg)",
    )
    parser.add_argument(
        "--theta-offset-deg",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="closure alpha-energy Gaussian FWHM (default: 0 keV)",
    )
    parser.add_argument(
        "--energy-threshold-keV",
        type=float,
        default=0.0,
        help="closure per-alpha measured-energy threshold (default: 0 keV)",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "use automatic y-axis limits instead of the fixed beam-spot "
            "comparison ranges"
        ),
    )
    parser.add_argument(
        ...
        help="shared alpha0 y-axis range (default: 0.192 0.2015)",
    )
    parser.add_argument(
        "--alpha1-ylim",
        type=float,
        ...
        help="shared alpha1 y-axis range (default: 0.520 0.602)",
    )
    parser.add_argument(
        "--direct-ylim",
        type=float,
        ...
        help="shared direct-channel y-axis range (default: 0.195 0.285)",
    )
...
def validate_args(
    ...
    if any(
        ...
        raise ValueError(
            "all response values must be finite and non-negative"
        )
    ...
    mixed_with_beam = sorted(
        (
            (infer_beam_fwhm_mm(path), path)
            for path in args.mixed_files
        ),
        key=lambda item: (item[0], item[1].name),
    )
    beam_values = np.asarray(
        [item[0] for item in mixed_with_beam],
        dtype=float,
    )
    if np.unique(np.round(beam_values, decimals=12)).size != beam_values.size:
        raise ValueError(
            "Duplicate beam FWHM values inferred from input filenames. "
            "Please pass only one file per beam-spot size."
        )

    mixed_paths = [item[1] for item in mixed_with_beam]
    return mixed_paths, beam_values


def _run(args, save=False, outdir="."):
    print(f"Script version: {SCRIPT_VERSION}")
    mixed_paths, beam_values = validate_args(args)
    configure_plot_style()

    if args.output is None:
        theta_tag = str(args.theta_granularity_deg).replace(".", "p")
        phi_tag = str(args.phi_granularity_deg).replace(".", "p")
        args.output = Path(
            f"scan_162_beamspot_theta{theta_tag}_phi{phi_tag}_fractions.png"
        )
    ...
    plot_truth = np.empty((n_points, 3), dtype=float)
    ...
    for index, (mixed_path, beam_fwhm) in enumerate(
        zip(mixed_paths, beam_values)
    ):
        print(
            "\n"
            f"[{index + 1}/{n_points}] "
            f"{mixed_path.name}  "
            f"(beam FWHM={beam_fwhm:g} mm)"
        )
        ...
        if np.sum(full_file_counts) <= 0.0:
            raise RuntimeError(
                f"{mixed_path}: no recognized 162 channel labels in "
                "mixed reaction tree"
            )
        full_file_truth = (
        ...
        plot_truth[index] = reference_truth
        print(
        ...
        print(
            ...
            f"FWHM={args.energy_fwhm_keV:g} keV, "
    ...
    if reference_truth is not None:
        print(
            "\nPlot truth (flat dotted reference line, taken from the first "
            "mixed file full-file fractions): "
            + np.array2string(
                reference_truth,
                precision=8,
                separator=", ",
    ...
    print("\nBeam points used:")
    for beam_fwhm, path in zip(beam_values, mixed_paths):
        print(f"  {beam_fwhm:>5g} mm : {path}")
...
def _default_args() -> argparse.Namespace:
    """Argparse defaults with placeholder positional args (for code calls)."""
    return parse_args(["_", "_", "_", "_"])


def scan_beamspot_response_162_channel_fit(
    seq_be_gs, seq_be2plus, direct, mixed_files,
    save=False, outdir=".", **overrides,
):
    """Beam-spot response scan of the 162-keV channel-fraction fit.

    Parameters
    ----------
    seq_be_gs, seq_be2plus, direct : str or pathlib.Path
        The three pure single-channel benchmark ROOT files (templates).
    mixed_files : sequence of str or pathlib.Path
        Mixed pseudodata ROOT files, one per beam-spot point. The beam FWHM
        of each is inferred from its filename (…beam_fwhm<val>mm…).
    save : bool, optional
    ...
    **overrides :
        Any parser option (e.g. bins=60, grid_size=201, output="my.png",
        auto_y_range=True). Names use underscores, matching argparse.

    Returns
    -------
    dict with keys: figure, beam_values, medians, lower95, upper95, truth,
    mixed_paths.
    """
    args = _default_args()
    ...
    args.mixed_files = [Path(p) for p in mixed_files]
