#!/usr/bin/env python3
"""Fit-statistics scan for the 162-keV Dalitz analysis.

The scan variable is N_fit, the number of accepted complete three-alpha
closure events used in the Bayesian template fit.

All N_fit points share:
    - the same shuffled generated-event ordering;
    - the same Gaussian energy-smearing random numbers;
    - the same benchmark templates;
    - the same detector-response conditions.

The selected samples are nested prefixes: a larger N_fit sample contains the
events used at smaller N_fit. This reduces unrelated point-to-point variation.

A known per-alpha Si threshold is applied identically to benchmark and closure.
Beam spot, energy resolution, and theta/phi granularity remain closure-only.
...
SCRIPT_VERSION = "2026-07-20-fit-statistics-v1"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Scan the dependence of the extracted 162-keV channel fractions "
            "on the accepted complete-three-alpha fit statistics N_fit. "
            "The same shuffled generated-event order is used for all points, "
            "so larger samples contain the smaller samples as nested prefixes."
        )
    )
    parser.add_argument(
        "seq_be_gs",
    ...
    parser.add_argument(
        ...
        metavar="N",
        help=(
            "accepted complete-three-alpha event counts used in the fit "
            "(default: 10000 30000 50000 100000 200000 300000 "
    ...
    parser.add_argument(
        ...
        default=0.0,
        help=(
            "closure-only polar-angle granularity Delta-theta "
            "(default: 0 deg)"
        ),
    )
    parser.add_argument(
        ...
        default=0.0,
        help=(
            "closure-only azimuthal-angle granularity Delta-phi "
            "(default: 0 deg)"
        ),
    )
    ...
    parser.add_argument(
        ...
        default=162,
        help=(
            "generated-event ordering seed shared by all N_fit points "
            "(default: 162)"
        ),
    )
    parser.add_argument(
        ...
        default=1620,
        help=(
            "closure energy-smearing seed shared by all N_fit points "
            "(default: 1620)"
        ),
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "scan_162_fit_statistics_fractions.png"
        ),
        help="fraction-scan output PNG",
    )
    parser.add_argument(
        "--linear-x",
        action="store_true",
        help=(
            "use a linear N_fit axis; the default is logarithmic"
        ),
    )
    parser.add_argument(
        "--auto-y-range",
    ...
    parser.add_argument(
        ...
        metavar=("YMIN", "YMAX"),
        default=(0.180, 0.220),
        help="fixed alpha0 y-axis range (default: 0.180 0.220)",
    )
    parser.add_argument(
        "--alpha1-ylim",
        type=float,
        ...
        metavar=("YMIN", "YMAX"),
        default=(0.540, 0.660),
        help="fixed alpha1 y-axis range (default: 0.540 0.660)",
    )
    parser.add_argument(
        "--direct-ylim",
        type=float,
        ...
        metavar=("YMIN", "YMAX"),
        default=(0.160, 0.240),
        help="fixed direct-channel y-axis range (default: 0.160 0.240)",
    )
...
def validate_args(
    ...
    if (
        n_values.size == 0
        or np.any(n_values <= 0)
    ):
        raise SystemExit(
            "--n-values must contain positive integers"
        )
...
def apply_known_threshold_to_benchmark(
    reconstructed: dict[str, np.ndarray],
    threshold_kev: float,
) -> dict[str, np.ndarray]:
    kinetic = np.asarray(
...
def plot_statistics_scan(
    ...
    for channel, axis in enumerate(
        ...
        label_x, label_y = (
            INPANEL_LABEL_POSITIONS[channel]
        )
        label_va = (
            "top"
            if channel == 2
    ...
    figure.supxlabel(
        r"Accepted complete-$3\alpha$ events, $N_{\mathrm{fit}}$",
        y=0.035,
        fontsize=15,
    )
    figure.supylabel(
...
def main() -> int:
    ...
    print(
        "Loading pure benchmark samples..."
    )
    pure_events: list[
        dict[str, np.ndarray]
    ] = []
    ...
    for channel, path in enumerate(
        ...
        print(
            ...
            f"quality accepted="
            f"{accepted['event_id'].size:,}, "
            f"generated="
            f"{pure_generated_counts[channel]:,}"
        )

    print(
        "Loading mixed closure sample..."
    )
    ...
    if (
        ...
        raise RuntimeError(
            "No recognized 162 channel labels "
            "in mixed reaction tree"
        )
    flat_truth = (
        full_file_counts
        / np.sum(
    ...
    for channel, events in enumerate(
        ...
        thresholded = (
            apply_known_threshold_to_benchmark(
                reconstructed,
                args.known_threshold_keV,
            )
        )
        if (
            thresholded["event_id"].size
            == 0
        ):
            raise RuntimeError(
                f"No benchmark events survive "
                f"{args.known_threshold_keV:g} keV "
                f"for {CHANNEL_KEYS[channel]}"
            )
        template_histograms[channel] = (
            dalitz_histogram(
                thresholded,
                args.bins,
                limits,
    ...
    closure_angle_source = (
        resolve_closure_angle_source(
            args
        )
    )
    reconstructed_closure, _ = (
        reconstruct_with_granularity(
            mixed_events,
            args.closure_theta_granularity_deg,
            args.closure_phi_granularity_deg,
    ...
    if maximum_requested > available:
        raise RuntimeError(
            f"Maximum requested N_fit="
            f"{maximum_requested:,}, but only "
            f"{available:,} closure events survive "
            "the fixed detector response."
        )
    ...
    print(
        f"  closure energy FWHM="
        f"{args.closure_energy_fwhm_keV:g} keV"
    )
    print(
    ...
    print(
        f"  closure angle source="
        f"{closure_angle_source}"
    )
    print(
    ...
    lower95 = np.empty_like(
        medians
    )
    upper95 = np.empty_like(
        medians
    )

    for index, n_fit in enumerate(
        n_values
    ):
        print(
            "\n"
        ...
        prefix_counts, prefix_truth = (
            generated_prefix_truth(
                shuffled_reaction_ids,
                generated_count,
                sorted_reaction_ids,
                sorted_reaction_channels,
