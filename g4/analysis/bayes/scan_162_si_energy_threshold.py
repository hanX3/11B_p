#!/usr/bin/env python3
"""Known Si-threshold scan for the 162-keV Dalitz fit.

The per-alpha Si threshold is treated as a calibrated, known detector
parameter and is therefore applied identically to both the pure benchmark
templates and the mixed closure data.

The following less-certain detector-response effects remain closure-only:
    - finite beam spot through sphere-center angle reconstruction;
    - Gaussian Si energy resolution;
    - theta and phi angular granularity.

At every threshold point, the three benchmark templates and their total
channel efficiencies are rebuilt after applying the same threshold. A
three-alpha event survives only when all three alpha energies are above the
threshold.
...
SCRIPT_VERSION = "2026-07-20-si-known-threshold-v2"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Scan the effect of a known per-alpha Si detector threshold on "
            "the extracted 162-keV channel fractions. The same threshold is "
            "applied to benchmark templates and mixed closure data. Beam spot, "
            "energy resolution, and angular granularity remain closure-only "
            "response effects."
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        metavar="THRESHOLD",
        help=(
            "known per-alpha Si thresholds in keV, applied identically to "
    ...
    parser.add_argument(
        ...
        default=100000,
        help=(
            "exact accepted complete three-alpha closure events at each "
            "threshold point (default: 100000)"
        ),
    )
    parser.add_argument(
        "--seed",
    ...
    parser.add_argument(
        ...
        default=1620,
        help=(
            "common standard-normal closure energy-smearing seed "
            "(default: 1620)"
        ),
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "scan_162_si_known_threshold_fractions.png"
        ),
        help="output PNG",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        metavar=("YMIN", "YMAX"),
        default=(0.190, 0.205),
        help="fixed alpha0 y-axis range (default: 0.190 0.205)",
    )
    parser.add_argument(
        "--alpha1-ylim",
        type=float,
        ...
        metavar=("YMIN", "YMAX"),
        default=(0.575, 0.605),
        help="fixed alpha1 y-axis range (default: 0.575 0.605)",
    )
    parser.add_argument(
        "--direct-ylim",
        type=float,
        ...
        metavar=("YMIN", "YMAX"),
        default=(0.195, 0.230),
        help="fixed direct-channel y-axis range (default: 0.195 0.230)",
    )
...
def validate_args(
    ...
    if (
        ...
        raise SystemExit(
            "--threshold-values-keV must contain finite non-negative values"
        )
    ...
    if any(
        ...
        raise SystemExit(
            "closure response parameters must be finite and non-negative"
        )
...
def apply_known_threshold_to_benchmark(
    ...
    """Apply the known threshold to an ideal benchmark sample."""
    kinetic = np.asarray(
        reconstructed["kinetic"],
        dtype=float,
    )
    ...
    return selected, survival


def main() -> int:
    args = parse_args()
    print(f"Script version: {SCRIPT_VERSION}")
    threshold_values = validate_args(args)
    ...
    print("  full angular coverage")
    print(
        "  known threshold: applied to benchmark and closure"
    )
    print(
        "  benchmark: continuous momentum angles, true energies, "
    ...
    ideal_reconstructed_benchmarks: list[
        dict[str, np.ndarray]
    ] = []
    for events in pure_events:
        ...
        ideal_reconstructed_benchmarks.append(
            reconstructed
        )
    ...
    for index, threshold_kev in enumerate(
        ...
        template_survivals = np.empty(
            3,
            dtype=float,
        )

        for channel, reconstructed in enumerate(
            ideal_reconstructed_benchmarks
        ):
            thresholded_benchmark, survival = (
                apply_known_threshold_to_benchmark(
                    reconstructed,
                    float(threshold_kev),
                )
            )
            template_survivals[channel] = survival
            efficiencies[channel] = (
                thresholded_benchmark["event_id"].size
                / float(
                    pure_generated_counts[channel]
                )
            )
            if (
                ...
                raise RuntimeError(
                    f"No benchmark events survive threshold "
                    f"{threshold_kev:g} keV for "
                    f"{CHANNEL_KEYS[channel]}"
                )
            template_histograms[channel] = (
                dalitz_histogram(
                    thresholded_benchmark,
                    args.bins,
                    limits,
                )
            )
        ...
        print(
            ...
            f"N_available="
            f"{int(diagnostics['available_after_threshold']):,}"
        )
        print(
    ...
    plot_fraction_scan(
        ...
        upper95,
        plot_truth,
        "Known per-alpha Si threshold (keV)",
        args.output,
