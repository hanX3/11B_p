#!/usr/bin/env python3
"""Bayesian fit scan for symmetric retained theta ranges and fixed phi range.

Each theta scan point is supplied with a repeated option such as

    --theta-range 10-80 100-170

The two retained theta intervals must be mirror symmetric about 90 degrees.
A known retained phi interval is supplied once, for example

    --phi-range 0-360
    --phi-range 300-60

The same theta and phi coverage is applied to all pure benchmark templates
and to the mixed closure data. All three alpha particles must lie inside both
the retained theta union and the retained phi interval.

At every theta-range point, the three benchmark Dalitz templates and total
channel efficiencies are rebuilt.
...
SCRIPT_VERSION = "2026-07-20-symmetric-theta-phi-ranges-fit-v3-boundary-labels"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Study the fitted 162-keV channel fractions for explicitly "
            "specified forward-backward-symmetric polar-angle coverage. "
            "Each --theta-range option supplies two retained theta intervals, "
            "for example: --theta-range 10-80 100-170. The same known coverage "
            "is applied to all benchmark templates and the mixed closure data."
        )
    )
    parser.add_argument(
        "seq_be_gs",
    ...
    parser.add_argument(
        ...
        help=(
            "one symmetric retained-coverage configuration, written as two "
            "LOW-HIGH strings. Repeat this option for multiple scan points. "
            "Example: --theta-range 10-80 100-170"
        ),
    )
    parser.add_argument(
        "--phi-range",
        type=str,
        default="0-360",
        metavar="PHI_START-PHI_END",
        help=(
            "known retained azimuthal-angle range applied identically to "
            "benchmark and closure. Wrapped ranges are allowed, for example "
            "300-60. Default: 0-360"
        ),
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "scan_162_symmetric_theta_ranges_fractions.png"
        ),
    ...
    parser.add_argument(
        ...
        metavar=("YMIN", "YMAX"),
        default=(0.10, 0.30),
        help="fixed alpha0 y-axis range (default: 0.10 0.30)",
    )
    parser.add_argument(
        "--alpha1-ylim",
        type=float,
        ...
        metavar=("YMIN", "YMAX"),
        default=(0.30, 0.70),
        help="fixed alpha1 y-axis range (default: 0.30 0.70)",
    )
    parser.add_argument(
        "--direct-ylim",
        type=float,
        ...
        metavar=("YMIN", "YMAX"),
        default=(0.10, 0.50),
        help="fixed direct-channel y-axis range (default: 0.10 0.50)",
    )
...
def parse_phi_interval_token(
    ...
    if match is None:
        raise SystemExit(
            ...
            "Use START-END, for example 0-360 or 300-60."
        )
    ...
    if (
        abs(raw_difference) >= 360.0 - 1.0e-12
...
def phi_particle_mask(
    phi_deg: np.ndarray,
    phi_interval: tuple[float, float, float],
) -> np.ndarray:
    start, _, width = phi_interval
    if width >= 360.0 - 1.0e-12:
        return np.ones_like(phi_deg, dtype=bool)

    relative = np.mod(phi_deg - start, 360.0)
    return relative <= width + 1.0e-12
...
def validate_args(
    ...
    configurations = parse_symmetric_theta_ranges(
        args.theta_range
    )
    args.parsed_phi_range = parse_phi_interval_token(
        args.phi_range
    )
...
def retained_angular_mask(
    ...
    phi_width = float(phi_interval[2])
...
def plot_fraction_scan_by_range(
    labels: list[str],
    median: np.ndarray,
    lower95: np.ndarray,
    upper95: np.ndarray,
    ...
    axes[-1].set_xticklabels(
        labels,
        rotation=20 if len(labels) > 4 else 0,
        ha="right" if len(labels) > 4 else "center",
    )

    # Hide the y-tick labels at the boundaries between adjacent panels.
    # The tick marks themselves are retained.
    figure.canvas.draw()
    for panel_index, axis in enumerate(axes):
        ...
        if panel_index < len(axes) - 1 and visible_labels:
            visible_labels[0].set_visible(False)
        if panel_index > 0 and visible_labels:
            visible_labels[-1].set_visible(False)

    figure.supxlabel(
        r"Retained laboratory polar-angle ranges "
    ...
    figure.subplots_adjust(
        left=0.13,
        right=0.98,
        bottom=0.13 if len(labels) > 4 else 0.09,
        top=0.985,
        hspace=0.02,
    )
    figure.savefig(
...
def main() -> int:
    ...
    print("  retained theta and phi ranges: benchmark and closure")
    print(
    ...
    print(
        f"  retained phi range={args.phi_range}, "
        f"width={args.parsed_phi_range[2]:g} deg"
    )
    print(
        "  full-file truth="
        + np.array2string(
    ...
    for point_index, intervals in enumerate(
        configurations
    ):
        label = theta_configuration_label(
            intervals
        )
        labels.append(label)
        ...
        for channel, events in enumerate(
            ...
            if benchmark_selected["event_id"].size == 0:
                raise RuntimeError(
                    ...
                    f"the retained theta ranges {label}."
                )
        ...
        if args.statistics_mode == "fixed-generated":
            ...
            if selected["event_id"].size == 0:
                raise RuntimeError(
                    f"No closure events survive {label} within the fixed "
