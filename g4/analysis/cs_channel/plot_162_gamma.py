...
Dual use:
    terminal :  python3 plot_162_gamma.py ON.root            # no off overlay
                python3 plot_162_gamma.py ON.root OFF.root    # angular on/off
                (saves the three PNGs)
    jupyter  :  from plot_162_gamma import plot_162_gamma
                fig_spec, fig_ang, fig_casc = plot_162_gamma("ON.root")
                fig_spec, fig_ang, fig_casc = plot_162_gamma("ON.root", "OFF.root")
...
STEM = "gamma_162"          # output filename stem
...
def load(path):
    with uproot.open(path) as f:
        return f["reaction"].arrays(FIELDS, library="np")
...
def plot_162_gamma(on_file, off_file=None, save=False,
                   a1=A1_DEF, a2=A2_DEF):
    """162-keV gamma-channel validation figures.

    Parameters
    ----------
    on_file : str or pathlib.Path
        Run with the gamma0 W(theta) angular distribution ON.
    off_file : str or pathlib.Path, optional
        Run with gamma0 isotropic (OFF). If given, the gamma0 angular
        figure overlays an off/isotropic baseline curve.
    save : bool, optional
        False (default) -> do not write files.
        True            -> write the three {STEM}_*.png files.
    a1, a2 : float, optional
        Legendre coefficients for the analytic W(theta) reference curve.

    Returns
    -------
    (fig_spectrum, fig_gamma0_angular, fig_cascade) : tuple of Figures
    """
    a_on = load(on_file)
    a_off = load(off_file) if off_file else None
    ...
    if m0.sum() + mc.sum() == 0:
        raise ValueError(
            "no gamma events (gamma_branch > 0) in the first file. "
            "Did you run validation_162_gamma_angular_on.mac?")
    ...
    # =====================================================================
    fig_casc, ax = plt.subplots(figsize=FIGSIZE)
    ...
    fig_casc.subplots_adjust(**FIXED_MARGINS)

    if save:
        fig_spec.savefig(f"{STEM}_spectrum.png", dpi=200)
        fig_ang.savefig(f"{STEM}_gamma0_angular.png", dpi=200)
        fig_casc.savefig(f"{STEM}_cascade_isotropy.png", dpi=200)
        print(f"wrote {STEM}_spectrum.png")
        print(f"wrote {STEM}_gamma0_angular.png")
        print(f"wrote {STEM}_cascade_isotropy.png")
    ...
    return fig_spec, fig_ang, fig_casc
...
if __name__ == "__main__":
    ...
    plot_162_gamma(args.on_file, args.off_file, save=True,
